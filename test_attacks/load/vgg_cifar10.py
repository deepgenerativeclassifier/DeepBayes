...
# file copied from https://github.com/geifmany/cifar-vgg/blob/master/cifar10vgg.py
...
class cifar10vgg:
    ...
    def build_model(self):
        ...
        model.add(Lambda(lambda x: tf.nn.softmax(x)))
        return model
...
if __name__ == '__main__':
    ...
    correct_prediction = (np.argmax(predicted_x,1) == np.argmax(y_train,1))
    accuracy = np.mean(correct_prediction)

    print("the training accuracy is: ", accuracy * 100)

    predicted_x = model.predict(x_test)
    correct_prediction = (np.argmax(predicted_x,1) == np.argmax(y_test,1))
    accuracy = np.mean(correct_prediction)
